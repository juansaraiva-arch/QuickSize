...
# =========================================================================
    ...
    class PDF(FPDF):
        def header(self):
            # Franja Amarilla CAT (R, G, B)
            self.set_fill_color(255, 205, 0) 
            self.rect(0, 0, 210, 20, 'F')
            
            # Título del Reporte
            self.set_font('Arial', 'B', 14)
            self.set_text_color(0, 0, 0)
            self.cell(0, 8, 'CAT Size Solution | Engineering Executive Brief', 0, 1, 'R')
            self.ln(10)
        ...
        def footer(self):
            ...
            self.cell(0, 10, f'Page {self.page_no()} - Generated by CAT Size Solution v2.1', 0, 0, 'C')
    ...
    def create_premium_pdf():
        ...
        pdf.cell(0, 10, f"Project: {dc_type}", 0, 1, 'L')
        
        pdf.set_font('Arial', '', 10)
        timestamp = pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')
        pdf.cell(0, 5, f"Date: {timestamp}   |   Region: {region}   |   Scenario: {bess_strategy}", 0, 1, 'L')
        pdf.ln(5)
        
        # --- 2. EXECUTIVE SUMMARY ---
        pdf.chapter_title("1. EXECUTIVE SUMMARY")
        summary_text = (
            f"This proposal outlines a high-availability power solution designed to meet a strict {avail_req}% uptime target "
            f"while managing extreme load fluctuations typical of AI workloads ({load_step_pct}% Load Step).\n\n"
            f"The selected configuration utilizes a {selected_config['name']} strategy, combining {selected_gen} generators "
            f"with BESS technology to optimize CAPEX and ensure transient stability."
        )
        pdf.chapter_body(summary_text)
        
        # --- 3. SYSTEM CONFIGURATION (TABLA) ---
        pdf.chapter_title("2. SYSTEM ARCHITECTURE")
        
        # Dibujamos una caja para los datos técnicos
        pdf.set_fill_color(255, 255, 255)
        
        pdf.kpi_row("Generator Model:", f"{selected_gen} ({gen_data['type']})")
        pdf.kpi_row("Configuration:", f"{n_running} Running + {n_reserve} Reserve ({n_total} Total Units)")
        pdf.kpi_row("Installed Capacity:", f"{installed_cap:.1f} MW")
        pdf.kpi_row("Net Efficiency (LHV):", f"{net_efficiency*100:.2f}% ({heat_rate_lhv_mj:.1f} MJ/kWh)")
        pdf.kpi_row("PUE Impact:", f"{pue:.2f} -> {pue_actual:.2f} (via {cooling_method})")
        pdf.ln(3)

        # --- 4. BESS & STABILITY ---
        pdf.chapter_title("3. BESS & STABILITY INTEGRATION")
        pdf.kpi_row("BESS Capacity:", f"{bess_power_total:.1f} MW / {bess_energy_total:.1f} MWh")
        pdf.ln(2)
        ...
        pdf.cell(10); pdf.cell(0, 5, f"{chk_transient} Transient Voltage Sag Mitigation", 0, 1)
        pdf.cell(10); pdf.cell(0, 5, f"{chk_step} Step Load Support ({bess_breakdown.get('step_support', 0):.1f} MW)", 0, 1)
        pdf.cell(10); pdf.cell(0, 5, f"{chk_spin} Spinning Reserve Replacement ({selected_config.get('spinning_from_bess', 0):.1f} MW)", 0, 1)
        pdf.cell(10); pdf.cell(0, 5, f"{chk_black} Black Start Capability", 0, 1)
        pdf.ln(5)

        # --- 5. FINANCIAL OVERVIEW ---
        pdf.chapter_title("4. FINANCIAL ANALYSIS (20-YEAR MODEL)")
        
        # Tabla simple financiera
        ...
        metrics = [
            ("Total Initial CAPEX", f"${total_capex_dynamic:.2f} Million"),
            ("Levelized Cost (LCOE)", f"${lcoe_dyn:.4f} / kWh"),
            ("Annual Savings vs Grid", f"${annual_savings/1e6:.2f} Million/year"),
            ("Net Present Value (NPV)", f"${npv_dyn/1e6:.2f} Million"),
            ("Simple Payback", payback_str_dyn)
        ]
        
        for metric, value in metrics:
            pdf.cell(95, 8, metric, 1, 0)
        ...
        pdf.multi_cell(0, 4, "DISCLAIMER: This report is a preliminary engineering estimate generated by the CAT Size Solution tool. "
                             "Final sizing, costs, and performance must be validated by a formal engineering study and Caterpillar detailed quotation. "
                             "Fuel prices, taxes, and interest rates are based on user inputs and subject to market changes.")

        return pdf.output(dest='S').encode('latin-1')

    # Botón de Descarga
    pdf_bytes = create_premium_pdf()
    
    st.sidebar.download_button(
        label="📄 Download Professional Report",
        data=pdf_bytes,
        file_name=f"CAT_Solution_{dc_type.replace(' ', '_')}_Premium.pdf",
        mime="application/pdf",
        help="Generate a client-ready engineering brief."
    )
